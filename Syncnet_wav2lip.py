...
class SyncNet(nn.Module):
    ...
    def __init__(self, in_channel_image,in_channel_audio, out_dim):
        ...
        self.merge_encoder_face = nn.Sequential(
            nn.Conv2d(out_dim, out_dim // 2, kernel_size=3, padding=1, stride=2),
            nn.LeakyReLU(0.2),
            nn.Conv2d(out_dim // 2, out_dim // 4, kernel_size=3, padding=1),
        )
        self.merge_encoder_audio = nn.Sequential(
            nn.Conv2d(out_dim, out_dim // 2, kernel_size=3, padding=1, stride=2),
            nn.LeakyReLU(0.2),
            nn.Conv2d(out_dim // 2, out_dim // 4, kernel_size=3, padding=1),
        )
    def forward(self, image,audio):
        ...
        audio_embedding = self.merge_encoder_audio(audio_embedding)
        image_embedding = self.merge_encoder_face(image_embedding)
...
class SyncNetPerception(nn.Module):
    ...
    def __init__(self, pretrain_path):
        ...
        if pretrain_path != "":
            ...
            print('train lip sync model from scratch')
